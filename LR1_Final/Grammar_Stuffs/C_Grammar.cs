    ...
    class C_Grammar
    {
        ...
        public void add_prodution_from_source(string left, string right)
        {
            ...
            foreach (string sub_production in productions)
            {
                ...
                foreach (string a_symbol in sub_tokens)
                { //Se recorre cada una los simbolos obtenidos para esta produccion.
                    if (string.Compare(a_symbol, string.Empty) != 0)
                    { //Solo se agregan simbolos obviamente, si no son cadenas vacias.
